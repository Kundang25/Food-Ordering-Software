...
import { ShoppingCart, Search, Menu, X, PanelLeftOpen } from "lucide-react";
...
const Navbar = () => {
  ...
  useEffect(() => {
    document.body.style.overflow = isOpen || isSidebarOpen ? "hidden" : "unset";
    return () => {
  ...
  return (
    <>
          ...
          <div className="flex justify-between items-center py-2">
            ...
            <div className="flex items-center">
              <span className="text-xs sm:text-base md:text-lg lg:text-xl font-bold text-black">
                <img src="./EasyEatsText.png" className="h-6 sm:h-8 lg:h-12"/>
              </span>
            </div>
            ...
            <div className="hidden sm:flex space-x-4">
              {["/HomePage", "/contact-us","/Combos","/receipts"].map((path, index) => (
                <button
                  key={index}
            ...
            <div className="hidden sm:flex items-center space-x-3">
              ...
              >
                <FontAwesomeIcon icon={faBars} className="w-5 h-5 text-gray-800" />
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Main Sidebar - visible when sidebar icon is clicked */}
      <div
      ...
      >
        ...
        <div className="flex flex-col px-6 py-4">
          ...
          <button
            onClick={() => handleNavigation("/CancellationAndRefund")}
            className="py-3 text-left text-gray-800 hover:text-orange-500 border-b border-gray-200"
          >
            CANCELLATION & REFUND
          </button>
      ...
      {/* Mobile Menu Sidebar - visible when hamburger is clicked */}
      <div
        className={`fixed inset-y-0 right-0 z-50 w-64 bg-white/90 backdrop-blur-sm shadow-lg transform transition-transform duration-300 ease-in-out ${
